...
module flex_stp_sr_NUM_BITS4_SHIFT_MSB0 ( clk, n_rst, shift_enable, serial_in, 
  ...
  DFFSR \parallel_out_reg[3]  ( .D(n14), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        parallel_out[3]) );
  DFFSR \parallel_out_reg[2]  ( .D(n12), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        parallel_out[2]) );
  DFFSR \parallel_out_reg[1]  ( .D(n10), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        parallel_out[1]) );
